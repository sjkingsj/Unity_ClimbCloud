...
\- Find 매서드
 \+ Find(오브젝트) : 오브젝트 이름과 일치하는 게임 오브젝트 한 개 반환
 \+ FindWithTag(태그이름) : 태그 이름과 일치하는 게임 오브젝트 한 개 반환
 \+ FindGameObjectsWithTag(태그이름) : 태그 이름과 일치하는 여러 개 반환
 \+ FindObjectOfType(타입이름) : 타입 이름과 일치하는 오브젝트 한 개 반환
 \+ FindObjectsOfType(타입이름) : 타입 이름과 일치하는 여러 개 반환
...
void Start()
{ this.rigid2D = GetComponent<Rigidbody2D>(); }  // AddForce 매서드 사용하기 위해서 Rigidbody2D 컴포넌트 구하기
void Update()
{
 // 점프
...
void Update()
{
 ...
 { this.rigid2D.AddForce(transform.right * key * this.walkForce); }  // 가속
 // 움직이는 방향에 따라 이미지 반전
 if (key != 0)
 { transform.localScale = new Vector3(key, 1, 1);  // 스프라이트의 X축 방향 배율을 1 or -1 배로
}
}
...
void Update()
{
 ...
 transform.position = new Vector3(transform.position.x, playerPos.y, transform.position.z);  // 카메라의 y축을 playerPos.y 로 변경
}
}
...
\+ Entry : 애니메이션을 시작할 때 Entry 노드로 전환
...
\+ Exit Time : 애니메이션 종료 시간을 정규화 시간(0~1)으로 설정
